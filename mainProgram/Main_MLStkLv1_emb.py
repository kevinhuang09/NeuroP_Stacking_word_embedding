...
testModelCount = 99  # 只取 5 個 model（lightgbm、xgboost 一定會被包含在內）
# ======================================================================================================================
...
# ======================================================================================================================
# 基本路徑：直接接 Main_FeatureStk_emb.py 產生的結果，不重新推導 feature type/欄位
# Main_FeatureStk_emb.py 對每個 normalizeMethod 都會把 feature filter 完的資料依 feature type 拆成
...
# 注意：featureStatPath/dataName 跟 Main_FeatureStk.py（word embedding 版）共用同一份輸出，
# 這裡讀到的永遠是「最後一次執行的那個版本」產生的結果；只有下面本檔案自己的輸出路徑（mlScorePath/tuneModelPath）
# 額外加上 _emb 後綴，跟 Main_MLStkLv1.py 的輸出分開存放，方便判斷這批結果是用 ESM/T5 embedding 跑出來的。
