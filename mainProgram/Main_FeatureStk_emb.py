...
def countEnabledFeatureType(featureDict):
    ...
    for groupName, groupDict in featureDict.items():
        if groupName == 'llmEmbeddingFeature':
            # llmEmbeddingFeature 底下同時有整個 type 的總開關 'Usage'，以及各方法各自的開關，
            # 跟其他「只有一個 Usage 代表整個 feature type」的群組不同，要特別處理：
            # 總開關關閉時整組都不算，開啟時逐一列出各方法各自有沒有開啟
            if groupDict.get('Usage') is True:
                for key, value in groupDict.items():
                    if key in ('Usage', 'modelDirPath', 'blockSize'):
                        continue
...
def buildAllOffFeatureDict(baseDict):
    """
    把 iFeature/pFeature/ampFeature/ovpFeature/llmEmbeddingFeature 底下每個 feature type 開關關閉，
    centerGDPFeature 整個關閉，保留其餘參數結構。
    llmEmbeddingFeature 的 'Usage' 是整個 type 的總開關（不是個別 feature type），維持原值不動，
    這樣之後才能個別把 ESM/T5 單獨打開來做欄位偵測（若把總開關也關掉，底下每個方法都會失效）。
    """
    offDict = copy.deepcopy(baseDict)
    for groupName in ['iFeature', 'pFeature', 'ampFeature', 'ovpFeature', 'llmEmbeddingFeature']:
        if groupName not in offDict:
            continue
        for key, value in offDict[groupName].items():
            if key in ('modelDirPath', 'Usage', 'blockSize'):  # 不是個別開關：modelDirPath 是路徑前綴，Usage 是總開關，blockSize 是 padding 長度
                continue
...
def getRealEnabledFeatureTypeList(featureDict):
    ...
    # llmEmbeddingFeature 的 'Usage' 是整個 type 的總開關，關閉時底下任何方法都不會真的產生欄位，
    # 所以要先檢查總開關，開啟時才逐一列出各方法各自的開關
    if featureDict['llmEmbeddingFeature'].get('Usage') is True:
        for key, value in featureDict['llmEmbeddingFeature'].items():
            if key in ('Usage', 'modelDirPath', 'blockSize'):
                continue
            if isinstance(value, list) and value[0] is True:
                enabledList.append(('llmEmbeddingFeature', key))
    return enabledList
...
# LLM embedding(ESM/T5)的 embedding 快取 csv 路徑前綴跟 dataName 有關，
# FeatureConfig.py 裡沒有 dataName 可用，故在這裡動態設定
featureDict['llmEmbeddingFeature']['modelDirPath'] = paramPath + f'{dataName}_llmEmbedding'

normalizeMethodList = ['standard', 'robust']  # normalization 目前先用standard 可自行改list
...
def getFeatureTypeDisplayName(typeName):
    ...
    其餘 feature type（例如 iFeature.AAC、llmEmbeddingFeature.ESM）維持原本取 key 當顯示名稱的邏輯。
    """
...
# LLM embedding(ESM/T5)是凍結參數的預訓練模型，不需要（也不能）用自己的資料集重新訓練，
# 但推論時的 padding 長度(blockSize)要統一，才能確保 DS_Train/DS_Indp/DS_Val 用同一套標準去算 embedding，
# 因此在這裡先算出全部資料集中最長序列的長度，動態設定進 llmEmbeddingFeature 的 blockSize
AllSeqDict = {**DS_TrainNegSeqDict, **DS_TrainPosSeqDict,
              **DS_IndpNegSeqDict, **DS_IndpPosSeqDict,
              **DS_ValNegSeqDict, **DS_ValPosSeqDict}
originalFeatureDict['llmEmbeddingFeature']['blockSize'] = max(len(seq) for seq in AllSeqDict.values())
